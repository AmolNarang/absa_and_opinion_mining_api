...
class MssqlHandler:
    ...
    def fetch_df(self):
        """This function will fetch the records from the db if no table exists
         means if the cursor is none it will give error"""

        if self.mssql_cursor.description is None:
            rem_df = f"No table found for the given operation"
        else:
            headers = [tup[0] for tup in self.mssql_cursor.description]
            rem_df = DataFrame.from_records(self.mssql_cursor.fetchall(), columns=headers)
